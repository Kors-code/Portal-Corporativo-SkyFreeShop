...
class PassengerExcelImportService
{
    ...
    public function importPath(string $path, string $filename, array $options = []): array
    {
        ...
        try {
            ...
            $storedTotals = $this->storedBatchTotals($batch->id);
            $rawRowsImported = $rowsImported;
            $rawTotalPax = round($totalPax, 2);
            $deduplicatedRows = $storedTotals['rows'];
            $deduplicatedTotalPax = $storedTotals['pax'];

            $dates = array_values(array_filter($dates));

            $batch->update([
                'status' => empty($errors) ? 'completed' : 'completed_with_warnings',
                'rows_imported' => $deduplicatedRows,
                'rows_skipped' => $rowsSkipped,
                'total_pax' => $deduplicatedTotalPax,
                ...
                'notes' => [
                    ...
                    'source_file_id' => $options['source_file_id'] ?? null,
                    'observed_scope' => $options['observed_scope'] ?? null,
                    'raw_rows_imported_before_dedupe' => $rawRowsImported,
                    'raw_total_pax_before_dedupe' => $rawTotalPax,
                    'deduplicated_rows_saved' => $deduplicatedRows,
                    'deduplicated_total_pax_saved' => $deduplicatedTotalPax,
                    'duplicate_rows_collapsed' => max(0, $rawRowsImported - $deduplicatedRows),
                    'duplicate_pax_collapsed' => round($rawTotalPax - $deduplicatedTotalPax, 2),
    ...
    private function storedBatchTotals(int $batchId): array
    {
        $row = PassengerFlight::where('batch_id', $batchId)
            ->selectRaw('COUNT(*) as rows_count, COALESCE(SUM(pax), 0) as pax_value')
            ->first();

        return [
            'rows' => (int) ($row?->rows_count ?? 0),
            'pax' => round((float) ($row?->pax_value ?? 0), 2),
        ];
    }
