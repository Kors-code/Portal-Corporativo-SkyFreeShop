        ...
        return $facts
            ...
            ->map(function (PassengerMonthlyFact $fact) {
                ...
                return [
                    ...
                    'source_mode' => $singleMonthBatches->isNotEmpty()
                        ? 'onedrive_single_month_batch_total'
                        : 'onedrive_flight_rows',
                    'difference_vs_flight_rows' => round($factPax - $flightPax, 2),
                    'status' => $fact->source_name === 'OneDrive Sky Free PAX Col' ? 'OK' : 'REVISAR',
                    'source_name' => $fact->source_name,
                    'source_period' => $fact->source_period,
                    'explanation' => $singleMonthBatches->isNotEmpty()
                        ? 'El total mensual se toma del batch OneDrive del mes, para respetar el total operativo del archivo.'
                        : 'El total mensual se reconstruye desde filas de vuelos OneDrive porque el archivo cubre mas de un mes o no hay batch mensual unico.',
                ];
            })
            ->all();
    }
