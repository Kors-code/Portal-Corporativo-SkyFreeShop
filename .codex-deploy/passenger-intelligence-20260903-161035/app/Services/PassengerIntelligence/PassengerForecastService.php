...
class PassengerForecastService
{
    ...
    private function monthlyTotalsUntil(int $targetYear, int $targetMonth): array
    {
        ...
        $this->applyObservedMonthlyPaxFacts($totals, $targetPeriod);

        return array_values($totals);
    }

    private function applyObservedMonthlyPaxFacts(array &$totals, string $targetPeriod): void
    {
        $facts = PassengerMonthlyFact::query()
            ->where('airport_iata', 'MDE')
            ->where('direction', 'total')
            ->where('fact_type', 'skyfree_commercial_observed_pax')
            ->where('source_type', 'skyfree_onedrive_pax')
            ->orderBy('year')
            ->orderBy('month')
            ->get();
        ...
        foreach ($facts as $fact) {
            ...
            $observedPax = round((float) $fact->value, 2);
            $current = $totals[$period] ?? [
                ...
                'colombian_pax' => 0.0,
                'foreign_pax' => 0.0,
                'flights' => (int) $fact->records_count,
            ];

            $colombianPct = $current['pax'] > 0 && $current['colombian_pax'] > 0
                ? round(($current['colombian_pax'] / $current['pax']) * 100, 3)
                : ($current['colombian_pct'] ?? null);

            $current['pax'] = $observedPax;
            $current['flights'] = max((int) ($current['flights'] ?? 0), (int) $fact->records_count);
            $current['pax_source'] = 'skyfree_onedrive_monthly_fact';

            if ($colombianPct !== null) {
                $current['colombian_pct'] = $colombianPct;
                $current['foreign_pct'] = round(100 - $colombianPct, 3);
                $current['colombian_pax'] = round($observedPax * ($colombianPct / 100), 2);
                $current['foreign_pax'] = round($observedPax - $current['colombian_pax'], 2);
            }
