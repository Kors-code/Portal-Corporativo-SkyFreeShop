...
class PassengerFlightEstimationService
{
    ...
    public function monthlyAnalytics(array $filters = []): array
    {
        ...
        return $query
            ...
            ->selectRaw('SUM(CASE WHEN estimates.composition_profile_id IS NULL THEN 1 ELSE 0 END) as missing_composition')
            ->selectRaw('COUNT(estimates.composition_profile_id) as rows_with_composition')
            ...
            ->map(function ($row) {
                ...
                $hasComposition = (int) $row->rows_with_composition > 0;
                $colombianPax = $hasComposition ? round((float) $row->colombian_pax, 2) : null;
                $foreignPax = $hasComposition ? round((float) $row->foreign_pax, 2) : null;
                ...
                return [
                    ...
                    'colombian_pax' => $colombianPax,
                    'foreign_pax' => $foreignPax,
                    'colombian_pct' => $commercialPax > 0 && $colombianPax !== null ? round(($colombianPax / $commercialPax) * 100, 3) : null,
                    'foreign_pct' => $commercialPax > 0 && $foreignPax !== null ? round(($foreignPax / $commercialPax) * 100, 3) : null,
                    'missing_composition' => (int) $row->missing_composition,
                    'rows_with_composition' => (int) $row->rows_with_composition,
    ...
    private function resolveComposition(PassengerFlight $flight): ?PassengerCompositionProfile
    {
        ...
        if ($date) {
            return $this->compositionCache[$cacheKey] = (clone $query)
                ->where(function ($q) use ($date) {
                    $q->whereNull('valid_from')->orWhereDate('valid_from', '<=', $date);
                })
                ->orderByRaw('CASE WHEN direction IS NULL THEN 1 ELSE 0 END')
