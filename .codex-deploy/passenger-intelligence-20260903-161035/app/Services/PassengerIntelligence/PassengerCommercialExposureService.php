...
class PassengerCommercialExposureService
{
    ...
    public function refreshObservedFacts(?int $year = null, ?int $month = null): array
    {
        ...
        $singleMonthBatchTotals = PassengerImportBatch::query()
            ->where('source_type', 'onedrive_skyfree_pax')
            ->where('observed_scope', 'commercial_flow')
        ...
        if ($year) {
            $singleMonthBatchTotals->whereYear('period_start', $year);
        }

        if ($month) {
            $singleMonthBatchTotals->whereMonth('period_start', $month);
        }

        $batchTotalRows = $singleMonthBatchTotals
            ->select(
                DB::raw('YEAR(period_start) as year_value'),
                DB::raw('MONTH(period_start) as month_value'),
                DB::raw("'total' as direction"),
                DB::raw('SUM(total_pax) as pax_value'),
                DB::raw('SUM(rows_imported) as records_count')
            )
        ...
        foreach ($totalRows as $row) {
            $totalRowsByPeriod[((int) $row->year_value) . '-' . ((int) $row->month_value)] = $row;
        }

        foreach ($batchTotalRows as $row) {
            $totalRowsByPeriod[((int) $row->year_value) . '-' . ((int) $row->month_value)] = $row;
        }

        $facts = [];

        foreach (array_merge($directionRows->all(), array_values($totalRowsByPeriod)) as $row) {
