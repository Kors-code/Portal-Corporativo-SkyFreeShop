...
class PassengerOneDrivePaxService
{
    ...
    public function reloadImportedFiles(PassengerExcelImportService $importer, ?int $userId = null, ?int $limit = null): array
    {
        $query = PassengerSourceFile::where('provider', 'onedrive')
            ->whereIn('status', ['discovered', 'imported', 'already_imported', 'import_failed'])
            ->orderBy('source_last_modified_at')
            ->orderBy('name');
        ...
        foreach ($files as $sourceFile) {
            ...
            try {
                ...
                $batch = PassengerImportBatch::where('source_file_id', $sourceFile->id)
                    ->orderByDesc('id')
                    ->first();

                if (!$batch) {
                    $batch = PassengerImportBatch::where('checksum', $checksum)
                        ->orderByDesc('id')
                        ->first();
                }
                ...
                $result = $batch
                    ? $importer->rebuildExistingBatch($batch, $path, $options)
                    : $importer->importPath($path, $sourceFile->name, $options);
                ...
                $results[] = [
                    ...
                    'total_pax' => $result['total_pax'] ?? 0,
                    'raw_total_pax_before_dedupe' => $result['raw_total_pax_before_dedupe'] ?? null,
                    'old_batch_pax' => $result['old_batch_pax'] ?? null,
                    'old_flight_rows_pax' => $result['old_flight_rows_pax'] ?? null,
                    'pax_difference' => $result['pax_difference'] ?? null,
                    'flight_rows_difference' => $result['flight_rows_difference'] ?? null,
                ];
            } catch (\Throwable $e) {
                $sourceFile->update([
                    'status' => 'import_failed',
        ...
        return [
            ...
            'files_reloaded' => count($results),
            'files_failed' => count($errors),
            'rows_imported' => (int) collect($results)->sum('rows_imported'),
            'total_pax' => round((float) collect($results)->sum('total_pax'), 2),
            'results' => $results,
            'errors' => $errors,
        ];
    }
