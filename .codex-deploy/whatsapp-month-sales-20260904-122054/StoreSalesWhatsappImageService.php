...
class StoreSalesWhatsappImageService
{
    ...
    public function make(array $report): string
    {
        ...
        $monthSales = (float) ($report['month_sales_usd'] ?? $totals['total_usd'] ?? 0);
        ...
        $this->drawFooter($image, $compliance, $monthSales, $meta, $this->updatedAtLabel($report), $isRange);
    ...
    private function drawFooter($image, float $compliance, float $monthSales, float $meta, string $updatedAtLabel, bool $isRange): void
    {
        $this->roundedRect($image, 34, 680, 410, 780, 14, $this->colors['white']);
        imagerectangle($image, 34, 680, 410, 780, $this->colors['line']);
        ...
        $this->roundedRect($image, 452, 680, 828, 780, 14, $this->colors['white']);
        imagerectangle($image, 452, 680, 828, 780, $this->colors['line']);
        $this->text($image, 'MONTH SALES', 476, 718, 12, $this->colors['muted'], true);
        $this->text($image, $this->usd($monthSales), 476, 754, 28, $this->colors['ink'], true);

        $this->roundedRect($image, 870, 680, 1246, 780, 14, $this->colors['white']);
        imagerectangle($image, 870, 680, 1246, 780, $this->colors['line']);
        $this->text($image, $isRange ? 'RANGE TARGET' : 'DAILY TARGET', 894, 718, 12, $this->colors['muted'], true);
        $this->text($image, $this->usd($meta), 894, 754, 28, $this->colors['ink'], true);
