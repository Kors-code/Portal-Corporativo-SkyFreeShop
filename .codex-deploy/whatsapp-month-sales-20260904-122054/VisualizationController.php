...
class VisualizationController extends Controller
{
    ...
    public function inventoryMonitoring(Request $request, InventoryReportService $service)
    {
        ...
        $maxMonths = filter_var($request->input('max_months', InventoryReportService::DEFAULT_MAX_MONTHS), FILTER_VALIDATE_INT, [
            'options' => ['min_range' => 1],
        ]);

        return response()->json([
            'generated_at' => now()->toISOString(),
            'rows' => $service->getReport($search, $storeIds, $asOfDate, min($maxMonths ?: InventoryReportService::DEFAULT_MAX_MONTHS, InventoryReportService::MAX_ALLOWED_MONTHS)),
        ]);
    }
    ...
    public function storeSalesReportData(Request $request): array
    {
        ...
        $monthStart = (new \DateTimeImmutable($endDate))->modify('first day of this month')->format('Y-m-d');
        $monthSales = (float) $this->budgetDB()->table('sales as s')
            ->whereBetween('s.sale_date', [$monthStart, $endDate])
            ->whereIn('s.pdv', array_keys($storeMap))
            ->sum(DB::raw('COALESCE(s.value_usd, 0)'));
        ...
        return [
            ...
            'meta_usd' => $meta,
            'month_sales_usd' => round($monthSales, 2),
