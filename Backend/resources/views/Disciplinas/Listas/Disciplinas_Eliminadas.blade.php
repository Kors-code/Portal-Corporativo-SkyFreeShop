...
<div id="modalSuccess" class="modal-overlay" style="display:flex;">
    <div class="modal-box">
        <h3>✅  Restaurado Correctamente</h3>
        <p>{{ session('success') }}</p>
...
@endif
<h1>📋 Disciplinas Positivas Eliminadas</h1>
<a href="{{ route('Disciplinas.list') }}" class="btn-primary">⬅️ Retroceder</a>

<form action="{{ route('disciplinas.eliminadas') }}" method="GET" class="filter-form">
        ...
        <tbody>
            @foreach ($LlamadoAtencion as $Llamado)
                <tr>
                    ...
                    <td><form action="{{ route('disciplinas.restore') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id" value="{{ $Llamado->id }}">
                        <button class="btn-primary btn-restore" type="button">Restaurar</button>
                    </form>
                    </td>
                </tr>
            @endforeach
        ...
        <div id="modalConfirm" class="modal-overlay" style="display:none;">
            <div class="modal-box">
                ...
                <div class="modal-actions">
                    ...
                    <button id="confirmBtn" class="btn-confirm">Aceptar</button>
                </div>
            </div>
        </div>
