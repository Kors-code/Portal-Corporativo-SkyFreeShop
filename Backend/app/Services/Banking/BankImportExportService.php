...
class BankImportExportService
{
    ...
    public function importAndExport(string $bankCode, UploadedFile $file, int $receiptStart, ?int $userId = null): array
    {
        ...
        if (! in_array($bankCode, ['davibank', 'davivienda', 'bancolombia'], true)) {
    ...
    private function fillDavibankSheet(Worksheet $sheet, array $headers, array $rows, \DateTimeImmutable $date, int $receipt): int
    {
        ...
        $this->fillDavibankReceipt($sheet, $date, $subtotalRow + 4, $receipt, $subtotalRow);
    ...
    private function writeDaviviendaWorkbook(array $headers, array $rows, int $receiptStart): string
    {
        ...
        foreach ($this->groupRows('davivienda', $rows) as $dateKey => $dayRows) {
            ...
            $this->fillDaviviendaReceipt($sheet, $date, $totalRow + 6, $receipt, $totalRow);
    ...
    private function movementRows(string $bankCode, int $batchId, array $rows, array $context): array
    {
        ...
        return array_map(function (array $row) use ($bankCode, $batchId, $context, $now): array {
            return [
                ...
                'withholding_source_amount' => $bankCode === 'davibank' ? ($row['VALOR_RETENCION'] ?? 0) : ($row['Ret. Fuente'] ?? 0),
                'withholding_vat_amount' => $bankCode === 'davibank' ? ($row['VALOR_RETEIVA'] ?? 0) : ($row['Ret. IVA'] ?? 0),
                'withholding_ica_amount' => $bankCode === 'davibank' ? ($row['RETEICA'] ?? 0) : ($row['Ret. ICA'] ?? 0),
                'vat_amount' => $bankCode === 'davibank' ? ($row['VALOR_IVA'] ?? 0) : ($row['Valor Iva'] ?? 0),
                'consumption_tax_amount' => $row['Imp Al Consumo'] ?? 0,
                'tip_amount' => $bankCode === 'davibank' ? ($row['VALOR_PROPINA'] ?? 0) : ($row['Valor Propina'] ?? 0),
    ...
    private function sumRows(string $bankCode, array $rows, string $kind): float
    {
        return array_reduce($rows, function (float $carry, array $row) use ($bankCode, $kind): float {
            return $carry + match ($kind) {
                'sale' => $bankCode === 'davivienda' ? (float) ($row['Valor Consumo'] ?? 0) : ($bankCode === 'bancolombia' ? (float) ($row['amount'] ?? 0) : (float) ($row['VALOR_COMPRA'] ?? 0)),
                'commission' => $bankCode === 'davivienda' ? (float) ($row['Valor Comisión'] ?? 0) : ($bankCode === 'davibank' ? (float) ($row['VALOR_COMISION'] ?? 0) : 0.0),
                'withholding' => $bankCode === 'davivienda' ? (float) ($row['Ret. Fuente'] ?? 0) + (float) ($row['Ret. IVA'] ?? 0) + (float) ($row['Ret. ICA'] ?? 0) : ($bankCode === 'davibank' ? (float) ($row['VALOR_RETENCION'] ?? 0) + (float) ($row['VALOR_RETEIVA'] ?? 0) + (float) ($row['RETEICA'] ?? 0) : 0.0),
                'income' => $bankCode === 'davivienda' ? (float) ($row['Valor Neto'] ?? 0) : ($bankCode === 'bancolombia' ? (float) ($row['amount'] ?? 0) : (float) ($row['NETO_ABONADO'] ?? 0)),
    ...
    private function movementDate(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $this->dateString($row['FECHA_COMPR'] ?? null), 'davivienda' => $this->dateString($row['Fecha Vale'] ?? null), 'bancolombia' => $row['movement_date'] ?? null, default => null }; }
    private function processDate(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $this->dateString($row['FECHA_PROCESO'] ?? null), 'davivienda' => $this->dateString($row['Fecha Proceso'] ?? null), default => null }; }
    private function depositDate(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $this->dateString($row['FECHA_ABONO'] ?? null), 'davivienda' => $this->dateString($row['Fecha de Abono'] ?? null), 'bancolombia' => $row['movement_date'] ?? null, default => null }; }
    private function accountNumber(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['NUMERO_CUENTA'] ?? null, 'davivienda' => '475670049406', 'bancolombia' => $row['account_number'] ?? null, default => null }; }
    private function transactionCode(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['NUM_TRANSACC'] ?? null, 'bancolombia' => $row['transaction_code'] ?? null, default => null }; }
    private function reference(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['ID_MOVIMIENTO'] ?? $row['NUM_COMP'] ?? null, 'davivienda' => $row['Vale'] ?? null, default => null }; }
    private function receiptNumber(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['CONSECUTIVO'] ?? null, 'davivienda' => $row['Vale'] ?? null, default => null }; }
    private function authorization(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['NUM_AUTORIZACION'] ?? null, 'davivienda' => $row['Número Autoriza'] ?? null, default => null }; }
    private function terminal(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['NUM_TERMINAL'] ?? null, 'davivienda' => $row['Terminal'] ?? null, default => null }; }
    private function network(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['CODIGO_RED'] ?? null, 'davivienda' => $row['Red'] ?? null, default => null }; }
    private function cardType(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['TIPTARJ'] ?? null, 'davivienda' => $row['Tipo Tarjeta'] ?? null, default => null }; }
    private function cardLastDigits(string $bankCode, array $row): ?string { $raw = match ($bankCode) { 'davibank' => $row['NUM_TARJETA'] ?? '', 'davivienda' => $row['Tarjeta Socio'] ?? '', default => '' }; $digits = preg_replace('/\D+/', '', (string) $raw); return $digits ? substr($digits, -4) : null; }
    private function description(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['NOMBREALMACEN'] ?? $row['UBICACION_TERM'] ?? null, 'davivienda' => $row['Tipo Tarjeta'] ?? null, 'bancolombia' => $row['description'] ?? null, default => null }; }
    private function movementType(string $bankCode, array $row): ?string { return match ($bankCode) { 'davibank' => $row['ABONO_DEVOL'] ?? null, 'davivienda' => 'A', 'bancolombia' => 'credit', default => null }; }
